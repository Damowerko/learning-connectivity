...
class ConnectivityOpt:
    ...
    def update_network_config(self, step_size, verbose=False):
        ...
        for i in range(self.agent_count):
            for j in range(self.agent_count):
                ...
                if i in self.comm_idcs and j in self.comm_idcs:
                    ...
                    constraints += [
                        Aij[i, j]
                        == Rxixj + dRdxi.T @ (xi_var - xi).T + dRdxj.T @ (xj_var - xj).T
                    ]
        ...
        if conn_new < 1e-5:
            if verbose:
                print(
                    f"optimization close to infeasibility ({conn_new}), updating nothing"
                )
            return conn_prev, False
    ...
    def maximize_connectivity(
        ...
        for it in range(max_its):
            ...
            if viz:
                ...
                axes[1].cla()
                axes[1].plot(
                    np.maximum(np.asarray([-hist, 0]) + l2_hist.shape[0] - 1, 0),
                    l2_line
                    @ np.asarray([[0, min(l2_hist.shape[0], hist) - 1], [1, 1]]),
                    "k",
                    lw=2,
                )
                axes[1].plot(l2_hist, "r", lw=2)
                axes[1].set_title(
                    f"m = {round_sf(abs(l2_line[0]),2)}, diff = {round_sf(hist_diff,2)}"
                )
                plt.tight_layout()
                plt.pause(0.01)
            ...
            if abs(l2_line[0]) < m_tol and hist_diff < h_tol:
                if verbose:
                    print(f"stopping criterion reached")
                break
