...
class PathLossModel:
    ...
    def __init__(
        self, print_values=True, t=0, n0=-70.0, n=2.52, l0=-53.0, a=0.2, b=6.0
    ):
    ...
    def calculate_range(self, max_range=100):
        return toms748(
            lambda a: self.predict_link(np.zeros((2,)), np.asarray([a, 0]))[0] - 1e-10,
            1,
            max_range,
        )
...
class PiecewisePathLossModel(PathLossModel):
    ...
    def __init__(
        ...
        PN0 = dbm2mw(self.N0)
        PT = dbm2mw(self.t)
        self.trans_dist = (PL0 * PT / (PN0 * special.erfinv(transition_rate) ** 2)) ** (
            1 / self.n
        )

        # find the slope of R(xi, xj) at the cuttoff distance; this will serve
        # as the slope of the linear section that decays to zero
        dRdd = PathLossModel.derivative(
            self, np.asarray([self.trans_dist, 0.0]), np.zeros((2,))
        )
        self.m = dRdd[
            0
        ].item()  # by construction dR/dy = 0; dR/dx = slope (change in distance)
    ...
    def predict(self, x):
        ...
        rate[dist_mask] = np.maximum(
            self.m * edm[dist_mask] + self.b, np.zeros(edm[dist_mask].size)
        )
