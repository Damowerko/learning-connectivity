...
def plot_config(
    ...
    if task_ids is None:
        ...
        id_mask = np.asarray(
            [True if id in task_ids else False for id in ids], dtype=bool
        )
    ...
    if routes is not None:
        ...
        cmap = cm.ScalarMappable(
            norm=Normalize(vmin=0.0, vmax=1.0, clip=True), cmap="YlOrBr"
        )
    ...
    if routes is None and rates is not None:
        cmap = cm.ScalarMappable(
            norm=Normalize(vmin=0.0, vmax=1.0, clip=True), cmap="plasma"
        )
        for i, j in [(i, j) for i in range(len(x)) for j in range(i + 1, len(x))]:
            if rates[i, j] == 0.0:
                continue
