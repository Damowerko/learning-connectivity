...
def compute_peaks(image, threshold_val=80, blur_sigma=1, region_size=7, view=False):
    out_peaks, blurred_img = lloyd.compute_peaks(
        image, threshold_val, blur_sigma, region_size
    )
    if view:
        fig, ax = plt.subplots()
...
def connectivity_from_CNN(
    ...
    for i in range(samples):
        ...
        cnn_imgs[i] = (
            model.evaluate(torch.from_numpy(input_image)).cpu().detach().numpy()
        )
...
def compute_coverage(image, params, viz=False):
    ...
    config = lloyd.sub_to_pos(
        params["meters_per_pixel"], params["img_size"][0], config_subs
    )
    ...
    while True:
        ...
        new_config = lloyd.lloyd_step(
            image, params["xy"], config, voronoi_cells, coverage_range
        )
        ...
        if viz:
            for i, cell in enumerate(voronoi_cells):
                ...
                circle_patches.append(
                    mpl.patches.Circle(config[i], radius=coverage_range)
                )
            ...
            ax.add_collection(p)
            ax.add_collection(
                mpl.collections.PatchCollection(circle_patches, ec="r", fc="none")
            )
...
def segment_test(args):
    ...
    if args.sample is None:
        ...
        print(
            f"provided sample index {args.sample} out of range of dataset with length {dataset_len}"
        )
        return
    else:
        idx = args.sample
    ...
    plt.show()
    print(
        f"optimal coverage computed for {dataset_file.name} test partition sample {idx}"
    )
...
def line_test(model_file, draws=1, save=True, steps=20):
    ...
    for i, dist in enumerate(dists):
        ...
        cnn_conn, x_cnn, cnn_pwr, cnn_img = connectivity_from_CNN(
            img, model, x_task, params, draws
        )
        ...
        ax.set_title(
            f"opt. ({opt_conn:.3f}, 0) | CNN ({cnn_conn:.3f}, {pwr_str})", fontsize=18
        )
        ...
        if save:
            data = {
                "filename": f"line_{i:02d}_{model_name}",
                "img": disp_img,
                "json": {
                    "task": {"config": x_task.tolist()},
                    "cnn": {
                        "config": x_cnn.tolist(),
                        "power": cnn_pwr,
                        "connectivity": cnn_conn,
                    },
                    "opt": {
                        "config": x_opt.tolist(),
                        "power": params["channel_model"].t,
...
def circle_test(model_file, task_agents=3, draws=1, save=True, steps=15):
    ...
    for i, rad in enumerate(rads):
        ...
        if save:
            data = {
                "filename": f"circle_{i:02d}_agents_{task_agents}_{model_name}",
                "img": disp_img,
                "json": {
                    "task": {"config": x_task.tolist()},
                    "cnn": {
...
def connectivity_main(args):
    connectivity_test(
        args.model, args.dataset, args.sample, args.save, args.train, args.nost
    )


def connectivity_test(
    model_file, dataset, sample=None, save=True, train=False, nost=False
):
    model = load_model_for_eval(model_file)
    if model is None:
        return
    ...
    cnn_conn, x_cnn, cnn_L0, cnn_img = connectivity_from_CNN(
        task_img, model, x_task, params, min_config=not nost
    )
    ...
    pwr_str = "0" if pwr_diff < 0.01 else f"{pwr_diff:.1f}"
    ax.set_title(
        f"{idx}: opt. ({opt_conn:.3f}, 0), CNN ({cnn_conn:.3f}, {pwr_str})", fontsize=14
    )
    ...
    if not save:
        ...
        data = {
            "filename": str(idx) + "_" + dataset_file.stem,
            "img": disp_img,
            "json": {
                "task": {"config": x_task.tolist()},
                "cnn": {
...
def stats_compiler(params, results_queue):
    ...
    print(
        f"{np.sum(cnn_feasible & ~opt_feasible)} cases where only the CNN was feasible"
    )
    print(f"{np.sum(cnn_morepower)} cases where the CNN required more transmit power")
    print(
        f"cnn power use:  mean = {np.mean(cnn_power):.2f}, std = {np.std(cnn_power):.4f}"
    )
    print(
        f"cnn agent diff: mean = {np.mean(agent_count_diff):.3f}, std = {np.std(agent_count_diff):.4f}"
    )
    print(
        f"absolute error: mean = {np.mean(absolute_error):.4f}, std = {np.std(absolute_error):.4f}"
    )
    print(
        f"percent error:  mean = {100*np.mean(percent_error):.2f}%, std = {100*np.std(percent_error):.2f}%"
    )
...
def compute_stats_test(
    ...
    if samples is not None:
        if samples > dataset_len:
            print(
                f"requested sample count ({samples}) > dataset length ({dataset_len})"
            )
            return
        else:
            dataset_len = samples
    ...
    results_proc = Process(
        target=stats_compiler, args=(results_proc_params, results_queue)
    )
    results_proc.start()
    ...
    for i in range(num_processes):
        p = Process(
            target=stats_computer, args=(dataset_params, sample_queue, results_queue)
        )
    ...
    for i in range(dataset_len):
        ...
        sample_dict["opt_count"] = np.sum(
            ~np.isnan(hdf5_file[mode]["comm_config"][i, :, 0])
        )
...
def parse_stats_test(stats_files, labels, save=True, nolog=False):
    if len(stats_files) != len(labels):
        print(
            f"number of stats files ({len(stats_files)}) must match number of labels ({len(labels)})"
        )
        return
    ...
    if save:
        ...
        np.savetxt(
            "transmit_power.csv", tmp_powers, fmt="%.1f", delimiter=",", header="power"
        )
    ...
    for label in labels:
        mask = (
            np.round(stats[label]["power"], 1) == p["channel_model"].t
        )  # all samples where CNN used default power
        agent_diffs += [
            (stats[label]["cnn_count"][mask] - stats[label]["opt_count"][mask]).astype(
                int
            )
        ]

    bins = np.arange(
        np.min(np.hstack(agent_diffs)), np.max(np.hstack(agent_diffs)) - 2, 1
    )
    ...
    avg_team_size = np.mean(
        np.hstack([stats[l]["cnn_count"] + stats[l]["opt_count"] for l in labels])
    )
    print(
        f"power: mean = {np.mean(np.hstack(powers)):.2f}, std = {np.std(np.hstack(powers)):.3f}"
    )
    print(
        f"diffs: mean = {np.mean(np.hstack(agent_diffs)):.4f}, std = {np.std(np.hstack(agent_diffs)):.3f}"
...
def compute_time_test(args):
        ...
        for sample_idx in range(samples):
            ...
            while True:
                x_task, x_comm = min_feasible_sample(
                    task_agents, params["comm_range"], bbx
                )
            ...
            print(
                f"\rtiming sample {sample_idx}/{samples} for team {total_agents} agent team\r",
                end="",
            )
    ...
    res["cnn"] = {
        "mean": np.mean(cnn_time, axis=1).tolist(),
        "std": np.std(cnn_time, axis=1).tolist(),
    }
    res["opt"] = {
        "mean": np.mean(opt_time, axis=1).tolist(),
        "std": np.std(opt_time, axis=1).tolist(),
    }
...
if __name__ == "__main__":
    ...
    line_parser.add_argument(
        "--draws", metavar="N", type=int, default=1, help="use best of N model samples"
    )
    line_parser.add_argument(
        "--steps",
        metavar="S",
        type=int,
        default=20,
        help="number of steps to take between min and max radius",
    )

    circ_parser = subparsers.add_parser(
        "circle", help="run circle test on provided model"
    )
    ...
    circ_parser.add_argument(
        ...
        type=int,
        default=3,
        help="number of agents in the circle",
    )
    circ_parser.add_argument(
        "--draws", metavar="N", type=int, default=1, help="use best of N model samples"
    )
    ...
    extrema_parser = subparsers.add_parser(
        "extrema",
        help="show examples where the provided model performs well/poorly on the given dataset",
    )
    ...
    extrema_parser.add_argument(
        "--best", action="store_true", help="look for best results instead of worst"
    )

    seg_parser = subparsers.add_parser(
        "segment",
        help="test out segmentation method for extracting distribution from image",
    )
    ...
    seg_parser.add_argument(
        "--isolate",
        action="store_true",
        help="show the CNN output without the input image overlayed",
    )
    seg_parser.add_argument(
        "--view", action="store_true", help="show each iteration of Lloyd's algorithm"
    )

    conn_parser = subparsers.add_parser(
        "connectivity", help="compute connectivity for a CNN output"
    )
    ...
    conn_parser.add_argument(
        "--train", action="store_true", help="draw sample from training data"
    )
    conn_parser.add_argument(
        "--draws", metavar="N", type=int, default=1, help="use best of N model samples"
    )
    conn_parser.add_argument(
        "--nost", action="store_true", help="disable removing redundant agents"
    )

    comp_parser = subparsers.add_parser(
        "compute_stats", help="compute performance statistics for a dataset"
    )
    comp_parser.add_argument("model", type=str, help="model")
    comp_parser.add_argument("dataset", type=str, help="test dataset")
    comp_parser.add_argument(
        "--train", action="store_true", help="run stats on training data partition"
    )
    comp_parser.add_argument(
        "--samples",
        type=int,
        help="number of samples to process; if omitted all samples in the dataset will be used",
    )
    comp_parser.add_argument(
        "--nosave", action="store_true", help="don't save connectivity data"
    )
    comp_parser.add_argument(
        "--jobs",
        "-j",
    ...
    parse_parser = subparsers.add_parser(
        "parse_stats", help="parse performance statistics saved by compute_stats"
    )
    parse_parser.add_argument(
        "--stats",
        type=str,
        help="stats.npy files generated from compute_stats",
        nargs="+",
    )
    parse_parser.add_argument(
        "--labels", type=str, help="labels to use with each stats file", nargs="+"
    )
    ...
    var_parser = subparsers.add_parser(
        "variation", help="show variation in model outputs"
    )
    ...
    comp_time_parser = subparsers.add_parser(
        "compute_time", help="compare CNN inference time with optimization time"
    )
    comp_time_parser.add_argument("model", type=str, help="model")
    comp_time_parser.add_argument(
        "--draws", metavar="N", type=int, default=1, help="use best of N model samples"
    )

    plot_time_parser = subparsers.add_parser(
        "plot_time", help="compare CNN inference time with optimization time"
    )
    plot_time_parser.add_argument(
        "datafile", type=str, help="JSON datafile with computation time stats"
    )
