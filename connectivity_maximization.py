...
def conn_max_test():
    ...
    co.maximize_connectivity(
        init_step_size=0.5, min_step_size=0.01, m_tol=1e-6, h_tol=1e-5, viz=False
    )
...
def scale_test():
    ...
    for i in range(agent_count.size):
        ...
        x_comm = circle_points(
            0.2, team_size
        )  # np.zeros((team_size, 2)) + np.random.normal(0.0, 0.05, (team_size, 2))
        ...
        print(
            "running {:2d} trials for {:2d} agents".format(test_trials, agent_count[i])
        )
